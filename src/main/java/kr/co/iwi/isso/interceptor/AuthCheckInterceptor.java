...
public class AuthCheckInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		this.authTokenCheck(request, response);

		return true;
	}

	private void authTokenCheck(HttpServletRequest request, HttpServletResponse response) throws Exception {
		
		System.out.println(request.getRequestURI());

		request.removeAttribute(Const.TOKEN_COOKIE);
		request.removeAttribute(Const.TOKEN_EMAIL);
		...
		for (Cookie cookie : cookies) {
			if (Const.TOKEN_COOKIE.equals(cookie.getName())) {
				acsToken = cookie.getValue();
			}
		}

		boolean isEmptyToken = StringUtils.isEmpty(acsToken);
		if (isEmptyToken) {
			throw new UnauthorizedException();
		}

		boolean isExpiredToken = TokenUtil.isExpired(acsToken);
		if (isExpiredToken) {
